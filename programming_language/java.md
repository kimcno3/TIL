...
`hashCode()` 메소드는 `equals()`와 함께 오버라이딩하여 사용하는 것이 일반적이다. 그 이유는 해시코드 규약을 보면 이해할 수 있다.

**해시코드 규약**
|메소드명|리턴값1|리턴값2|
|--|--|--|
|`equals()`|`true`|`false`|
|`hashCode()`|`true`|`true` or `false`|

위 해시코드 규약을 참고했을 때, `eqauls()`가 `true`라면 `hashCode()` 또한 `true`를 리턴해야 한다.

하지만 `equals()`만 오버라이딩한 경우에는 두 객체가 논리적으로 동일한 객체임을 의미하더라도, `hashCode()`는 아직 두 객체의 물리적인 메모리 주소를 비교해 다르다는 결과를 내게 된다.

그렇기 때문에 해시코드 규약을 지키기 위해선 `hashCode()`의 오버라이딩도 필요하다.
