...
2. **실행 엔진(Execution Engine)**
    ...
    - **JIT 컴파일러("Just In Time" Compiler)**
        ...
        - 바이트 코드 해석 전에 중복 코드를 체크한 뒤, 인터프리터가 코드 변환을 우선으로 실시한다.
        - 코드변환 중간에 미리 체크한 중복코드가 나온다면, 해당 결과값을 미리 캐싱하고 같은 중복코드가 나올 때마다 캐싱값을 가져온다.
...
출처 : https://homoefficio.github.io/2018/10/13/Java-%ED%81%B4%EB%9E%98%EC%8A%A4%EB%A1%9C%EB%8D%94-%ED%9B%91%EC%96%B4%EB%B3%B4%EA%B8%B0/
...
## :pushpin: main 함수 실행 순서

모든 자바 클래스 코드에서 main 메소드가 가장 위에 작성되는 경우가 많다. 

이 부분에서 개인적인 생각으로는 클래스의 필드와 메소드가 먼저 선언된 다음, main() 메소드가 실행되어야 하는 것이 맞지 않은가에 대한 의문이 들었고, 자바스크립트에서 호이스팅 개념이 있듯이 자바에서도 비슷한 개념이 있나 궁금증이 생겨 찾아본 결과, 간단한 해답을 찾았다.

바로 자바로 구성된 프로그램, 파일을 실행시에는 **main() 메소드가 가장 먼저 실행된다**는 것이다.

즉, `main()` 메소드가 가장 먼저 실행되면서 해당 클래스의 객체를 생성하고 그 이후로 생성된 객체의 메소드를 호출하는 순서로 코드가 진행되는 것이다.
...
## :pushpin: 가비지 콜렉터(Garbage Collector)
...
그렇기 때문에 계산 오류를 막기 위해서는 (byte), (short) 코드를 연산식 앞에 추가해주면서 형변환을 지정해줘야 자동 타입 변환을 방지하여 예제에서 원하는 값을 구할 수 있습니다.
...
short 타입으로 생성된 `256`과 `255`를 byte로 캐스팅 하는 경우, 0과 -1값이 나오는데 이런 경우를 오버플로우라고 하고, 두 경우를 자세히 살펴보면 다음과 같습니다.
...
위의 설명의 예제로 나온 `255(1111 1111)`가 byte 타입에서 `-1`인 이유를 이해하지 못했다.
...
**그렇기 때문에 255(2진수로 1111 1111)를 byte로 표현했을 때 -1로 표현된다.**
